...
# @Last Modified time: 2018-06-30 18:16:07
import time
import re
import json
...
def CreateMsgGB(strMsg,strComModType,strBNo,strMProtVer,StrUProtVer,strSendDate,strCenterAddr,strDeviceAddr,strAppDataLen,strCommand,strAppData):
    ...
    #1、数据包启动符（2字节）：64H,64H,数据包的第一、二字节,为固定值64,64(十进制)--40,40(十六进制)
    BeginStr = "4040"
    #11、结束符（2字节，35H,35H）结束符'##' 2字节,数据包的最后两个字节,为固定值35,35(十进制)--23,23(十六进制)
    EndStr = "2323" #结束符（2字节，35H,35H）
    #2、业务流水号（2字节）：从00H开始  ；数据包第3、第4个字节；在发送命令时顺序加1。 以65565为模取循环
    BNo = strBNo
    #strMainProtVer = strMProtVer+StrUProtVer                        #3、协议版本号（2字节）：01H,01H     ；数据包第5字节，第6字节。
    MasterProtocolVer = strMProtVer                                 #协议版本号（2字节）：01H,01H 协议版本主版本号
    UserProtocolVer = StrUProtVer                                   #协议版本号（2字节）：01H,01H 协议版本用户版本号
    SendDate = strSendDate                                          #数据包发送时间标签（6字节）：000000000000；数据包第7字节-第12字节：秒、分、时、日、月、年各一个字节。
    ...
    OriginalAddr = strOriginalAddr                                  #5、源地址（6字节）：数据包第13字节-第18字节，传输装置发监控中心（上行）：01H00H00H00H00H00H，代表1#传输装置所发；02H00H00H00H00H00H，代表2#传输装置所发；。。。。。。；监控中心发传输装置：00H00H00H00H00H01H，代表1#监控中心所发；02H00H00H00H00H02H，代表2#监控中心所发；。。。。。。以此类推；。
    DestinationAddr = strDestinationAddr                            #6、目的地址（6字节）：数据包第19字节-第24字节，传输装置发监控中心（上行）：00H00H00H00H00H01H，代表发给1#监控中心；00H00H00H00H00H02H，代表发给2#监控中心；监控中心发传输装置：01H00H00H00H00H00H，代表发给1#传输装置，02H00H00H00H00H00H，代表发给2#传输装置，FFH FFH FFH FFH FFH FFH代表发给全部传输装置。
    ...
    Command = strCommand                                            #8、命令字节（1字节）：第27字节；具体定义见国标26875.3-2011的表2。
    AppData = strAppData                                            #应用数据单元 9、应用数据单元（根据传送数据包类型定长度）
    CheckSumStr = '1'                                               #10、校验和（1字节）；
    lsReturn = BeginStr + ResortStr(BNo) + strMainProtVer + strSendDate+OriginalAddr+DestinationAddr+AppDataLength+strCommand+strAppData +EndStr
    return lsReturn
