...
# @Last Modified time: 2018-03-27 05:47:07
import time
import re
import json
...
APPDataStruct = {
	...
	"BNoLowbit" : "2|3",					#业务流水号低字节 1字节,低字节先传
	"BNoHighbit" : "3|4",					#业务流水号高字节 1字节,高字节后传
	"BNo" : "2|4",							#业务流水号 2字节,BNo = BNoHighbit+BNoLowbit
    ...
    "AppDataLength" : "int(AppDataLengthHighbit+AppDataLengthlowbit,16)",  		#应用数据单元长度高字节 1字节
    "AppData" : "27|27+AppDataLength",      #应用数据 共AppDataLength长度个字节
    "Command" : "26|27",                    #命令字节 1字节

    "Checksum" : "-3|-2" ,                 	#校验和 1位 使用各位和校验和求反方式, 倒数第三位
    "BeginStr" : "-2|"						#结束符'##' 2字节,数据包的倒数第一、二字节,为固定值35H,35H(十进制)--23,23(十六进制)
}
...
APPDataStruct = {
    ...
    "AppObjBodyDefine" : [
	    [
	    	{
		    	...
		    	'''
		    	AppSysStatusBin = ("0000000000000000" + bin(int(AppSysStatus,16))
	    ...
	    [
	    	{
	    		"AppSysType" : "2|3", 	#9.2.1.2.2 建筑消防设施系统类型标志, 1字节,定义如国标26875.3-2011的表4(即,JsonAppSysType中定义的相关内容)
		    	"AppSysAddr" : "3|4", 	#9.2.1.2.2 系统地址, 1字节,消防设施部件设定
